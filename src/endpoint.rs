...
impl Profile {
    pub fn new(base_context: &chatex::BaseContext) -> Profile {
        ...
        me.path_segments_mut()
            .unwrap()
            .push("me");
        let mut balance = base_context.base_url.clone();
        balance.path_segments_mut()
            .unwrap()
            .push("me")
            .push("balance");
        Profile {
...
impl Exchange {
    ...
    pub fn new(base_context: &chatex::BaseContext) -> Exchange {
        ...
        exchange_url.path_segments_mut()
            .unwrap()
            .push(Self::EXCHANGE);
        let mut orders = exchange_url.clone();
        orders.path_segments_mut()
            .unwrap()
            .push(Self::ORDERS);
        let mut my = orders.clone();
        my.path_segments_mut()
            .unwrap()
            .push(Self::MY);
        let mut trades = orders.clone();
        trades.path_segments_mut()
            .unwrap()
            .push(Self::TRADES);
        Exchange {
            orders,
